  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-animated">
      ...
      <div className="absolute inset-0 z-10">
        <div className="floating absolute top-20 left-10 w-2 h-2 bg-primary rounded-full glow-primary"></div>
        <div className="floating-delayed absolute top-40 right-20 w-3 h-3 bg-secondary rounded-full glow-secondary"></div>
        <div className="floating absolute bottom-32 left-1/4 w-2 h-2 bg-accent rounded-full glow-accent"></div>
        <div className="floating-delayed absolute top-1/3 right-1/3 w-2 h-2 bg-electric rounded-full glow-electric"></div>
      </div>
      ...
      <div className={`relative z-20 text-center max-w-4xl mx-auto px-6 transition-all duration-1000 ${
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-center">
          <Button 
            className="btn-neon-primary text-lg py-6 px-10 hover:animate-pulse-neon"
            onClick={() => document.getElementById('lineup')?.scrollIntoView({ behavior: 'smooth' })}
          >
            Entrar na Experiência
          </Button>
          <Button 
            className="btn-neon-secondary text-lg py-6 px-10"
